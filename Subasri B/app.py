...
from ui_components import (inject_custom_css, inject_tab_css, render_story_block,
                            ...
                            generate_book_pdf, _extract_characters)
...
def merge_characters(new_names):
    existing = set(c.split("—")[0].strip().split()[0] for c in st.session_state.characters)
    for name in new_names:
        if name not in existing:
            st.session_state.characters.append(name)
            existing.add(name)

def auto_extract_story_info(story_text: str):
    """
    After AI generates a passage, call Ollama to extract:
    - Characters with their roles (only if user hasn't manually filled characters)
    - World/context inferences (only if user hasn't manually filled world context)
    Returns immediately, updates session state in place.
    """
    chars_locked   = st.session_state.get("chars_manual", False)
    world_locked   = st.session_state.get("world_manual", False)
    if chars_locked and world_locked:
        return  # user manually filled both — don't overwrite either

    # Build what we need to extract
    tasks = []
    if not chars_locked:
        tasks.append("CHARACTERS: List every named character that appears. Format exactly as:\nName — their role/description in one short phrase\nOne per line. If no named characters, write NONE.")
    if not world_locked:
        tasks.append("WORLD: In 1-3 short bullet points, infer the setting, time period, and any unique rules of this world (magic, technology, etc). If it's a plain real-world setting, write NONE.")

    if not tasks:
        return

    prompt = f"""Read this story passage and extract the following. Be concise and accurate.
...
Reply with ONLY the extracted information in the exact format above. No commentary."""
    ...
    try:
        ...
        if not user_has_chars and "CHARACTERS:" in result:
            char_block = result.split("CHARACTERS:")[1]
            if "WORLD:" in char_block:
                char_block = char_block.split("WORLD:")[0]
            lines = [l.strip() for l in char_block.strip().splitlines() if l.strip() and l.strip() != "NONE"]
            existing = set(c.split("—")[0].strip().lower() for c in st.session_state.characters)
            for line in lines:
                if "—" in line or "-" in line:
                    # normalise dash
                    line = line.replace(" - ", " — ").replace("- ", " — ")
                    name = line.split("—")[0].strip()
                    if name.lower() not in existing and len(name) > 1:
                        st.session_state.characters.append(line)
                        existing.add(name.lower())

        # Parse WORLD section
        if not user_has_context and "WORLD:" in result:
            world_block = result.split("WORLD:")[1].strip()
            lines = [l.strip().lstrip("•-* ") for l in world_block.splitlines()
                     if l.strip() and l.strip() not in ("NONE", "None")]
            if lines:
                st.session_state.story_context = "\n".join(lines)

    except Exception:
        pass  # extraction is best-effort — never block story flow
...
def process_story(instruction, action_type="continue"):
    ...
    payload={"model":st.session_state.ollama_model,
        ...
        "stream":True,"options":{"temperature":0.82,"top_p":0.90,"repeat_penalty":1.06,"num_predict":500}}
    with st.spinner(""):
        try:
            ...
            if full:
                ...
                # Smart extraction — characters with roles + world context
                auto_extract_story_info(full)
...
def workshop_query(prompt,result_type):
    ...
    payload={"model":st.session_state.ollama_model,
        ...
        "stream":True,"options":{"temperature":0.7,"num_predict":600}}
    with st.spinner(""):
        try:
            resp=requests.post(f"{OLLAMA_URL}/api/chat",json=payload,stream=True,timeout=180)
            resp.raise_for_status()
            ...
            for line in resp.iter_lines():
                ...
                try:
                    ...
                    if tok:
                        full+=tok; cc+=1
                        if cc%8==0: ph.markdown(f'<div class="nf-streaming">{full}▌</div>',unsafe_allow_html=True)
                    if c.get("done"): break
                except: continue
            ph.empty()
...
def load_history_entry(entry):
    ...
    st.session_state["chars_manual"] = False  # never lock on load — let Ollama re-extract
    st.session_state["world_manual"] = False
...
pct=min(100,int(st.session_state.session_words/max(st.session_state.writing_goal,1)*100))
...
if st.session_state.active_tab=="forge":
    ...
    with cs[4]:
        models=get_models()
        if models:
            idx=models.index(st.session_state.ollama_model) if st.session_state.ollama_model in models else 0
    ...
    with left:
        ...
        with left_panel:
            ...
            if new_goal != st.session_state.writing_goal:
                ...
                st.rerun()
            pct2=min(100,int(st.session_state.session_words/max(st.session_state.writing_goal,1)*100))
    ...
    with center:
        ...
        if not st.session_state.story_blocks: render_empty_state()
            ...
            for i,block in enumerate(st.session_state.story_blocks):
                render_story_block(block,i,on_chars_found=merge_characters,on_book_saved=auto_save)
    ...
    with right:
        ...
        with right_panel:
            # ── CHARACTERS ──────────────────────────────────────────
            chars_filled = bool(st.session_state.characters)
            chars_auto   = chars_filled and not st.session_state.get("chars_manual", False)
            if chars_auto:
                char_label = 'CHARACTERS <span style="font-size:7px;letter-spacing:.08em;color:#b8860b;font-family:var(--fu);margin-left:6px">✦ AUTO</span>'
            elif chars_filled:
                char_label = 'CHARACTERS <span style="font-size:7px;letter-spacing:.08em;color:#2a7a4b;font-family:var(--fu);margin-left:6px">✦ SET</span>'
            else:
                char_label = 'CHARACTERS'
            st.markdown(f'<p class="nf-panel-title">{char_label}</p>', unsafe_allow_html=True)
            ...
            if char_raw is not None:
                ...
                if new_chars != st.session_state.characters:
                    ...
                    st.session_state["chars_manual"] = True
            if st.session_state.characters:
                if st.button("✕ CLEAR", key="clr_chars", use_container_width=True):
                    st.session_state.characters = []
                    st.session_state["chars_manual"] = False
                    st.rerun()
            st.markdown('<hr style="border:none;border-top:1px solid #e2dbd0;margin:12px 0">',unsafe_allow_html=True)

            # ── WORLD / CONTEXT ─────────────────────────────────────
            world_filled = bool(st.session_state.story_context.strip())
            world_auto   = world_filled and not st.session_state.get("world_manual", False)
            if world_auto:
                world_label = 'WORLD / CONTEXT <span style="font-size:7px;letter-spacing:.08em;color:#b8860b;font-family:var(--fu);margin-left:6px">✦ AUTO</span>'
            elif world_filled:
                world_label = 'WORLD / CONTEXT <span style="font-size:7px;letter-spacing:.08em;color:#2a7a4b;font-family:var(--fu);margin-left:6px">✦ SET</span>'
            else:
                world_label = 'WORLD / CONTEXT'
            st.markdown(f'<p class="nf-panel-title">{world_label}</p>', unsafe_allow_html=True)
            ...
            if new_context != st.session_state.story_context:
                ...
                st.session_state["world_manual"] = True
            ...
            for lbl,fkey in [("OVERCOME BLOCK","overcome_block"),("FIX PACING","fix_pacing"),("ADD TENSION","add_tension"),("ADD SPECIFICS","add_specifics"),("DEEPEN EMOTION","deepen_emotion"),("VARY STYLE","vary_style")]:
                if st.button(lbl,key=f"fx_{fkey}",use_container_width=True): fix_action=fkey
            st.markdown('<hr style="border:none;border-top:1px solid #e2dbd0;margin:12px 0">',unsafe_allow_html=True)
    ...
    elif fix_action and st.session_state.story_blocks: process_story(fix_map.get(fix_action,f"__{fix_action}"),fix_action)
    elif fix_action: st.warning("Write something first.")
